...
function getGeminiClient(): GoogleGenAI | null {
  ...
  if (!apiKey) {
    console.warn('GEMINI_API_KEY is not set. Fallback analysis generator will be used if needed.');
    return null;
  }
...
async function generateWithModelFallback(
  ...
  modelsToTry: string[] = ['gemini-2.5-flash', 'gemini-2.5-pro']
): Promise<string> {
  ...
  for (const model of modelsToTry) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        const config: any = {
          temperature: promptOptions.temperature ?? 0.25,
        };
        ...
        const isTransient = errMsg.includes('503') || errMsg.includes('429') || errMsg.includes('UNAVAILABLE') || errStatus === 'UNAVAILABLE';

        if (isTransient && attempt === 1) {
          // Quick wait before 2nd attempt on transient spike
          await new Promise((r) => setTimeout(r, 600));
          continue;
        }

        console.warn(`[Gemini Fallback Engine] Model ${model} (attempt ${attempt}) encountered error, trying next available model:`, errMsg);
        break;
      }
    }
  }
...
function generateSmartFallback(answers: any[], candidateName?: string, userRole?: string) {
  ...
  if (isTech && isCreative) {
    primaryProf = {
      ...
      dayInTheLife: 'Утро начинается с дизайн-ревью в Figma, днем — интервью с пользователями и проектирование сценариев нового мобильного банка, вечером — полировка дизайн-системы и передача макетов разработчикам.',
    ...
    primaryProf = {
      ...
      dayInTheLife: 'Анализ массивов данных, обучение языковой модели на GPU-кластере, тестирование качества генерации ответов и интеграция модели в мобильное приложение компании.',
      firstStepsToStartNow: [
    ...
    primaryProf = {
      ...
      dayInTheLife: 'Проведение утреннего командного синка, анализ графика конверсий в покупку, формулирование гипотезы для нового функционала и общение с инвесторами.',
      firstStepsToStartNow: [
  ...
  return {
    ...
    psychologicalProfile: {
      ...
      archetypeDescription: 'Ты обладаешь выраженной способностью видеть взаимосвязи между сложными системами, быстро схватываешь суть вещей и ценишь профессиональную свободу. Тебе важно, чтобы деятельность приносила осязаемый интеллектуальный или визуальный результат.',
      traits: {
...
// POST endpoint for analyzing career test
app.post('/api/analyze-career', async (req, res) => {
  try {
    const { answers, candidateName, userRole } = req.body;
    ...
    if (!ai) {
      console.log('Using smart local fallback generation...');
      const fallbackResult = generateSmartFallback(answers, candidateName, userRole);
      return res.json(fallbackResult);
    }

    // Build the user prompt adhering precisely to user prompt requirements
  ...
  "recommendations": [
    {
      ...
      "matchScore": 96, // число совпадения от 75 до 99
  ...
  "motivationQuote": "Цитата великого человека или вдохновляющая мысль"
    ...
    const rawText = await generateWithModelFallback(
      ai,
      {
        ...
        temperature: 0.2,
        systemInstruction: 'Ты — ведущий профессиональный эксперт по профориентации и карьерный психолог. Формируй глубокий, реалистичный, строго структурированный и логически безупречный анализ на основе ответов пользователя, связывая каждый вывод с его выбором.',
      },
    ...
    try {
      ...
      console.warn('Failed to parse direct JSON from Gemini, cleaning markdown wrappers...', parseError);
    ...
    console.error('Error generating career guidance with Gemini API:', error);
    // Return high-quality fallback gracefully so user experience is not disrupted
    const fallback = generateSmartFallback(req.body?.answers || [], req.body?.candidateName, req.body?.userRole);
    return res.json(fallback);
  }
});

// Counselor & AI Assistant interactive chat endpoint
app.post('/api/ask-counselor', async (req, res) => {
  try {
    const { question, userContext, history, candidateName, userRole, language } = req.body;
      ...
      if (Array.isArray(history) && history.length > 0) {
        const cleanHistory = history.filter(
          (m) =>
            ...
            m.content &&
            m.id !== 'msg_welcome' &&
            !String(m.id).startsWith('msg_welcome_') &&
            m.id !== 'welcome'
        );
...
Дай прямой, содержательный, точный и структурированный ответ на этот вопрос:`;
      ...
      try {
        const timeoutPromise = new Promise<string>((_, reject) =>
          setTimeout(() => reject(new Error('AI_TIMEOUT')), 16000)
        );

        const generatePromise = generateWithModelFallback(
          ai,
          {
            contents: prompt,
            systemInstruction,
        ...
        if (generatedText && generatedText.trim()) {
          return res.json({ reply: generatedText.trim() });
        }
      } catch (geminiError) {
...
**Рекомендация:** Если хочешь детальный пошаговый план действий под твой текущий уровень — уточни свой класс/курс или конкретную цель!`;
    ...
    console.error('Error in ask-counselor endpoint:', error);
    return res.json({
      reply: 'Спасибо за вопрос! Чтобы получить подробный ответ, уточни, в каком направлении ты хочешь развиваться (программирование, дизайн, поступление или навыки), и я составлю индивидуальный план.',
    });
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Профориентация AI Server running on http://localhost:${PORT}`);
  });
}
